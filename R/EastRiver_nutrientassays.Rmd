...
nit_means$conc <- (nit_means$abs_corr - 0.1433890) / 0.2351738
amm_means$conc <- (amm_means$abs_corr - 0.083951923) / 0.009596154
phos_means$conc <- (phos_means$abs_corr - 0.05399663) / 0.01163993
...
nit_means$conc[nit_means$conc < 0] <- 0
amm_means$conc[amm_means$conc < 0] <- 0
phos_means$conc[phos_means$conc < 0] <- 0
...
nit_means$conc_corr <- nit_means$conc * (0.01/0.33) * (0.001) #no dilution for any of the extractions

# Ammonium in ppm; need to convert to mmol/g
amm_means$conc_corr[amm_means$extraction=="h2o"] <- amm_means$conc[amm_means$extraction=="h2o"] * (0.01/0.33) * (1/18.039)  #no dilution
amm_means$conc_corr[amm_means$extraction=="pp"] <- amm_means$conc[amm_means$extraction=="pp"] * (0.01/0.33)  * (1/18.039)  #no dilution
amm_means$conc_corr[amm_means$extraction=="dith"] <- amm_means$conc[amm_means$extraction=="dith"] * (0.01/0.33) * 10 * (1/18.039) #1:10 dilution
amm_means$conc_corr[amm_means$extraction=="hcl"] <- amm_means$conc[amm_means$extraction=="hcl"] * (0.01/0.33) * (1/18.039) #no dilution

# Phosphate in uM; convert to mmol/g (1um = 0.001mm)
phos_means$conc_corr[phos_means$extraction=="h2o"] <- phos_means$conc[phos_means$extraction=="h2o"] * (0.01/0.33) * (0.001) #no dilution
phos_means$conc_corr[phos_means$extraction=="pp"] <- phos_means$conc[phos_means$extraction=="pp"] * (0.01/0.33) * (0.001) #no dilution
phos_means$conc_corr[phos_means$extraction=="dith"] <- phos_means$conc[phos_means$extraction=="dith"] * (0.01/0.33) * (0.001) * 100 #1:100 dilution
phos_means$conc_corr[phos_means$extraction=="hcl"] <- phos_means$conc[phos_means$extraction=="hcl"] * (0.01/0.33) * (0.001) * 10 #1:10 dilution
...
```{r}
# Units mmol/g
colnames(nit_all_merge)[c(3:6)] <- c("nit_abs", "nit_abs_corr", "nit_conc","nit_conc_corr")
colnames(amm_all_merge)[c(3:6)] <- c("amm_abs", "amm_abs_corr", "amm_conc","amm_conc_corr")

inorgN_all_merge <- merge(nit_all_merge, amm_all_merge)
inorgN_all_merge$inorgN_sum <- -999
inorgN_all_merge$inorgN_sum <- inorgN_all_merge$nit_conc_corr + inorgN_all_merge$amm_conc_corr

all_n_merge <- merge(inorgN_all_merge, TN_samples, by=c("ID", "extraction"))

all_n_merge$TON <- -999
all_n_merge$TON <- all_n_merge$mol_n_per_g - all_n_merge$inorgN_sum
...
# Melt data
n_melt <- melt(all_n_merge, id=c("ID", "extraction", "location", "rep", "top_cm", "btm_cm", "dilution_tn", "depth_category"))
...
# Stacked bar plots
n1 <- ggplot(subset(n_melt, depth_category %in% "shallow" & variable %in% c("mol_n_per_g", "TON"))) +
  ...
  theme(axis.text = element_text(size = 25),
    ...
    scale_fill_manual(name="",
                      breaks=c("TON", "mol_n_per_g"),
                      labels=c("Organic", "Inorganic"),
                      values=c("black","#0072B2"))

n2 <- ggplot(subset(n_melt, depth_category %in% "subsurface" & variable %in% c("mol_n_per_g", "TON"))) +
  aes(y=value, x=extraction2, fill=variable) +
  geom_bar(stat="identity") +
  coord_flip() +
  xlab("Subsurface") + ylab(expression(Nitrogen~(mmol~g^-1))) +
  theme_bw() +
  theme(axis.text = element_text(size = 25),
        axis.title = element_text(size = 35),
        panel.grid.major = element_blank(),
...
## Plots
```{r}
ggplot(nit_all_merge) +
  aes(x=top_cm, y=nit_conc_corr) +
  geom_point() +
  ylab("nitrate (uM/g soil)") +
  facet_wrap(~extraction, scales = "free")

ggplot(amm_all_merge) +
  aes(x=top_cm, y=amm_conc_corr) +
  geom_point() +
  ylab("ammonium (mg/L/g soil)") +
  facet_wrap(~extraction, scales = "free")

ggplot(phos_all_merge) +
  aes(x=top_cm, y=conc_corr) +
  geom_point() +
  ylab("phosphate (uM/g soil)") +
  facet_wrap(~extraction, scales = "free")
```