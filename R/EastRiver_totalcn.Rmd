...
er_redo <- read.table("Data/EA_Data/CA_180920.txt", sep="\t")[-c(1:7, 16),] #Mariela re-ran these samples Sept 2018; removing all rows except samples
...
total_er_cn <- merge(sample_key, er_totalC_av, by="ID", all.y=TRUE)
...
total_er_cn_noNA <- left_join(total_er_cn, int_comp, by="ID") %>%
  ...
  select(ID, location, rep, top_cm, btm_cm, fraction, composite, date, C_mean_wt_percent, N_mean_wt_percent, C_sd, N_sd)
