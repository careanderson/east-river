...
p1 <- ggplot(subset(flux_total, Location == "MCT2-1")) +
  aes(x=datetime, y=flux.av) + #, color=Environment
  geom_point() +
  geom_smooth() +
...
ggplot(subset(flux_total, flux.av < 6.7)) +
...
## Combine with river height data for figure
```{r}
...
## Flux, redox, and water table depth
```{r}
...
ggplot(subset(flux_watertable, depth_cm %in% 15)) +
  aes(x=Moisture, y=flux.av, color=date) + #, color=Environment
  geom_point() +
  geom_smooth() +
  xlab("Soil moisture (volumetric)") + ylab("Soil respiration (CO2 flux)") +
  theme_bw() +
