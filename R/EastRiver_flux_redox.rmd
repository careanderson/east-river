...
p2 <- ggplot(rhiz_toc_av) +
  aes(x=as.POSIXct(date), y=TOC_mean, color=depth_class) +
...
p4 <- ggplot(rhiz_icp_av[which(rhiz_icp_av$date < "2018-08-10 12:00:00"), ]) +
  aes(x=as.POSIXct(date), y=Fe_mean, color=depth_class) +
#  scale_x_datetime(limits = lims) + 
  geom_point() + geom_line() +
  ylab("Fe (M)") +
  guides(color=guide_legend(title="Soil depth (cm)")) +
...
p5 <- ggplot(rhiz_icp_av[which(rhiz_icp_av$date < "2018-08-10 12:00:00"), ]) +
  aes(x=as.POSIXct(date), y=Mn_mean, color=depth_class) +
#  scale_x_datetime(limits = lims) + 
  geom_point() + geom_line() +
  ylab("Mn (M)") +
  guides(color=guide_legend(title="Soil depth (cm)")) +
...
p6 <- ggplot(subset(redox_av.means)) +
  aes(x=datetime, y=redox_av.corrected, color=as.factor(depth_cm)) +
#  scale_x_datetime(limits = lims) + 
  geom_point() + geom_smooth() +
  ylab("Eh (mV)") +
...
acclima_color <- (c("#F8766D","#7CAE00", "#00BFC4", "#C77CFF"))

p7 <- ggplot(acclima_av) +
  aes(x=date, y=moisture_mean, color=depth_cm) +
#  scale_x_datetime(limits = lims) + 
  geom_point(size = 1) +
  xlab("Time") + ylab("% moisture (vol)") +
  guides(color=guide_legend(title="Soil depth (cm)")) +
  facet_wrap(~location, labeller = as_labeller(position_names)) +
  theme_bw() +
  theme(axis.text.y = element_text(size = 12),
        ...
        legend.position = "none") +
  xlim(lims) +
  scale_color_manual(name="", values=acclima_color)
...
g <- rbind(g2, g3, g4, g5, g6, g7, size="first")
