...
p1 <- ggplot(subset(flux_total, Location %in% c("MCT2-1","MCT2-3"))) +
  aes(x=datetime, y=flux.av) +
...
p4 <- ggplot(subset(rhiz_toc_av, position_mct2 %in% c(1,3) & depth_class %in% c("30-40", "60-80", "80-100"))) +
...
p5 <- ggplot(subset(rhiz_icp_av, position_mct2 %in% c(1,3) & depth_class %in% c("30-40", "60-80", "80-100"))) +
...
# Stack the plots (flux, moisture/wtd, redox)
g1 <- ggplotGrob(p1)
g2 <- ggplotGrob(p2)
g3 <- ggplotGrob(p3)
...
# Stack the plots (NPOC, IC, metals)
g4 <- ggplotGrob(p4)
g5 <- ggplotGrob(p5)
g6 <- ggplotGrob(p6)
g7 <- ggplotGrob(p7)

g <- rbind(g4, g5, g6, g7, size="first")
g$widths <- unit.pmax(g4$widths, g5$widths)
grid.newpage()
grid.draw(g)
```