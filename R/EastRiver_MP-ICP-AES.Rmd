...
# Convert to e.g., mmol Fe / g soil (divide by formula weight; divide by 1000 to cancel mg;  multiply by 1000 to get into mmol)
...
# # Melt the data
mp_melt <- melt(mp_total, id=c("ID", "element", "extraction", "dilution", "other", "location", "rep", "top_cm", "btm_cm", "depth_category")) #taking out extration
...
## Combine MP data with total C, N data
```{r}

total_bulk_cn <- read.csv("Data/EA_data/25April2018_EA_bulk_processed.csv")
...
ggplot(subset(total_icp_cn, `Element.Label` %in% "Fe" & depth_category %in% c("shallow", "subsurface"))) +
  aes(x=Concentration_corrected, y=C_wt_percent) +
  geom_point(aes(size=3, color=depth_category)) +
  geom_smooth(method="lm") +
#  ggtitle("Total C in bulk soil vs. Fe ppm by extraction type") +
  xlab(expression(Iron~(mmol~g^-1))) + ylab("Soil C (weight %)") +
  facet_wrap(~extraction, scales="free") +
  theme_bw() +
...
#  ggtitle("Total C in bulk soil vs. Al ppm by extraction type") +
  xlab(expression(Aluminum~(mmol~g^-1))) + ylab("Total C in bulk soil (%)") +
...
fe_c_water <- subset(total_icp_cn, `Element.Label` %in% "Fe" & extraction %in% "Water")
fe_c_pp <- subset(total_icp_cn, `Element.Label` %in% "Fe" & extraction %in% "Pyrophosphate")
fe_c_hcl <- subset(total_icp_cn, `Element.Label` %in% "Fe" & extraction %in% "HCl")
fe_c_dith <- subset(total_icp_cn, `Element.Label` %in% "Fe" & extraction %in% "Dithionite")
...
# HCl
plot(fe_c_hcl$Concentration_corrected, fe_c_hcl$C_wt_percent)
fe_hcl_model <- lm(C_wt_percent ~ Concentration_corrected, data=fe_c_hcl)
abline(fe_hcl_model, col = "red")
summary(fe_hcl_model)

fe_hcl_shallow_model <- lm(C_wt_percent ~ Concentration_corrected, data=fe_c_hcl_shallow)
summary(fe_hcl_shallow_model)

fe_hcl_subsurface_model <- lm(C_wt_percent ~ Concentration_corrected, data=fe_c_hcl_subsurface)
